...
var queries = map[string]string{
	...
	"getOrderEmail": `select DISTINCT ON (item_id) orders.created_at, orders.updated_at, orders.amount, items.item_name, users.first_name, users.last_name, stores.store_name
                    ...
                    where orders.updated_at > to_timestamp(concat(current_date , ' 08:00:00'), 'YYYY-MM-DD hh24:mi:ss')
										and orders.updated_at < to_timestamp(concat(current_date , ' 23:59:59'), 'YYYY-MM-DD hh24:mi:ss')
	...
	"getLatestOrders": `
											...
											FROM
											    inventory.orders
											where user_uuid = ?
											and updated_at > to_timestamp(concat(current_date , ' 08:00:00'), 'YYYY-MM-DD hh24:mi:ss')
											and updated_at < to_timestamp(concat(current_date , ' 23:59:59'), 'YYYY-MM-DD hh24:mi:ss')
											ORDER BY
											    item_id,
											    updated_at DESC;`,
}
